...
class message_history extends rcube_plugin
{
    	...
    	public function init()
	{
		...
		if (true) {
			// add button and hook for the settings page that allows the admin user to select users that can view the logs page
		}
	...
	public function logs_form()
	{
		...
		//$output .= $table->show();
		//$output .= html::br();
		$output = html::p('h2', "Roundcube Logs");
		$output .= html::br();
	...
	public function log_message($args)
	{
		...
		$result = $db->query("INSERT INTO $table (from_user_name, to_user_name, subject, time_sent, modified) VALUES (?, ?, ?, ?, ?)",
			$from_name, $to_name, $subject, $date, $db->now());
		if ($db->is_error($result))
		{
			rcube::raise_error([
				'code' => 605, 'line' => __LINE__, 'file' => __FILE__, 
