...
class message_history extends rcube_plugin
{
	...
	//Initialization function, where to define hooks to be used	
	public function init()
	{
		...
		// install user hooks
		//Hook to Add Exercise Selection to Headers and to log sent message to the roundcybe message history table
		$this->add_hook('message_ready', array($this, 'log_sent_message'));
		//Hook to add sent message when email is sent through stackstorm and to mark any email as read when a user reads it
		...
		//if compose page is rendered, the dropdown will be added to the compose page to select an exercise
		if ($rcmail->action == 'compose') {
			$this->add_hook('render_page', array($this, 'add_dropdown_to_compose'));
		}

		// if user has the required permissions, the user will be able to view the message history table
	...
	//Function to register plugin handler and to set the page title for the Roundcube message history table
	...
	//Function to create a new roundcube section and to create a html table for message history
	...
	public function logs_form()
	{
		...
		//Get required information from the message_history_v2 database table
		...
		//Adding information from the database table to the html table based on the added header
	...
	//Function to add an exercise selection dropdown to the compose page with the views the user is part of
	
	public function add_dropdown_to_compose($args)
	{
		$rcmail = rcmail::get_instance();
		$this->rc = &$rcmail;
		//Information to make an api call to obtain the views the current user is part of
		...
		//if the current template shown is the compose template, a dropdown will be added with the available user views
	...
	//Function to add the Exercise header to the message and to log the new message record if it was sent through Roundcube
	...
	public function log_sent_message($args)
	{
		...
		//Insert the new message record into the database message_history_v2 table
		...
		//if the message couldn't be inserted into the database table, error out
		if ($db->is_error($result))
		{
			rcube::raise_error([
				'code' => 605, 'line' => __LINE__, 'file' => __FILE__, 
	...
	//Function to add a new record if the message was sent through Stackstorm and to mark a message as read
	...
	public function log_read_message($args)
	{
		...
		//add the new message record when the user reads the message, if it was sent through stackstorm	
		foreach ($to_array as $to) {
			$to = trim($to);
			//first check if the email record exists (if it was sent through Roundcube)
			$check_record_exists = $db->query("SELECT * FROM $table WHERE roundcube_message_id = '$message_id' AND to_user_name = '$to'");
			//if the record doesn't exist, insert the new record into the database table
			if ($check_record_exists->rowCount() === 0){
				$new_record = $db->query("INSERT INTO $table (from_user_name, to_user_name, subject, time_sent, modified, read_status, roundcube_message_id, exercise) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
				$from, $to, $parsed_subject, $timestamp, $db->now(), ($to === $user ? 'TRUE' : 'FALSE'), $message_id, $exercise);	
			}
			
			//make sure only the record is marked as read for the user who is actually reading the message
